...
filename = '/Users/swapna/Desktop/LaneDetection/test.mov'
class Lane:
        """
		...
		# This will hold the image after perspective transformation
                ...
                self.roi_points = np.float32([
                        (720,780), # Top-left corner
                        (150, 1079), # Bottom-left corner            
                        (1630,1079), # Bottom-right corner
                        (1040,780)]) # Top-right corner
		...
		"""
			...
			self.center_offset)[:7]+' cm', (int((5/600)*self.width), 
			int((40/338)*self.height)), cv2.FONT_HERSHEY_SIMPLEX, 
		...
		# Store left and right lane pixel indices
				...
				left_fit[1]*nonzeroy + left_fit[2] - margin)) & (nonzerox < (left_fit[0]*(
				nonzeroy**2) + left_fit[1]*nonzeroy + left_fit[2] + margin))) 
		
                right_lane_inds = ((nonzerox > (right_fit[0]*(nonzeroy**2) + 
				right_fit[1]*nonzeroy + right_fit[2] - margin)) & (nonzerox < (right_fit[0]*(
        ...
        def get_line_markings(self, frame=None):
                ...
                # don't appear to be nice, pure, solid colors (like white or yellow lane lines.)       
        ...
        def perspective_transform(self, frame=None, plot=False):
                ...
                self.warped_frame = cv2.warpPerspective(frame, 
                                                	self.transformation_matrix, self.orig_image_size, 
                                                	flags=(cv2.INTER_LINEAR)) 
	
                # Convert image to binary
                (thresh, binary_warped) = cv2.threshold(self.warped_frame, 
                ...
                if plot == True:
                        ...
                        warped_plot = cv2.polylines(warped_copy, 
        ...
        def plot_roi(self, frame=None, plot=False):
                ...
                this_image = cv2.polylines(frame, 
                                	np.int32([self.roi_points]), 
                                	True, (147,20,255), 3)
...
def lane_detect(original_frame):
